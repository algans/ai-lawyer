...
describe("POST /api/payment/callback", () => {
  ...
  it("[SECURITY] fake success flag in body cannot unlock document — verification is server-side only", async () => {
    // Even if someone posts a token that maps to a real payment, server-side callbackDogrula must return basarili:true
    callbackDogrula.mockResolvedValue({ basarili: false, iyzicoRef: "" });
    paymentFindFirst.mockResolvedValue({ id: "p1", documentId: "d1" });
    // Attacker tries to inject extra fields — they are ignored; only callbackDogrula result matters
  ...
  it("updates payment iyzicoRef to real paymentId on success", async () => {
    callbackDogrula.mockResolvedValue({ basarili: true, iyzicoRef: "real-pay-id-123" });
    paymentFindFirst.mockResolvedValue({ id: "p1", documentId: "d1" });
    const res = await POST(formReq("tok1") as any);
